...
class Policy(nn.Module):
    ...
    def calculate_loss(self, gamma=0.999):
        ...
        for (log_prob, value), Gt in zip(saved_actions, returns):
            # Calculate the policy loss, which the baseline is the value function
            advantage = Gt - value.item()
            policy_losses.append(-log_prob * advantage)
            # Calculate the value loss
            value_losses.append(F.smooth_l1_loss(value, torch.tensor([Gt], dtype=torch.double)))

        policy_loss = torch.stack(policy_losses).sum()
        value_loss = torch.stack(value_losses).sum()

        # Calculate the loss (= policy loss + value loss)
    ...
    for i_episode in count(1):
        ...
        if ewma_reward > 120:
            if not os.path.isdir("./preTrained"):
                os.mkdir("./preTrained")
            torch.save(model.state_dict(), './preTrained/LunarLander_lr{}_gamma{}.pth'.format(lr, gamma))
...
if __name__ == '__main__':
    ...
    train(lr, gamma)
    test(f'LunarLander_lr{lr}_gamma{gamma}.pth')