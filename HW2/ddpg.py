...
writer = SummaryWriter(f"./tb_record_{env_name}")
...
class Actor(nn.Module):
    def __init__(self, hidden_size, num_inputs, action_space):
        ...
        self.linear3 = nn.Linear(hidden_size//2, num_outputs)
    ...
    def forward(self, inputs):
        ...
        x = self.relu(self.ln1(self.linear1(inputs)))
        x = self.relu(self.ln2(self.linear2(x)))
...
class Critic(nn.Module):
    ...
    def forward(self, inputs, actions):
        ...
        x = self.relu(self.ln1(self.linear1(x)))
        x = self.relu(self.ln2(self.linear2(x)))
        x = self.linear3(x)
        return x
...
def train():   
    ...
    for i_episode in range(num_episodes):
        ...
        while True:
            ...
            reward = torch.tensor([[reward / 10.0]], dtype=torch.float32)
    ...
    agent.save_model(save_name, '.pth')        
...
if __name__ == '__main__':
    ...
    # actor_path = "preTrained\ddpg_actor_Pendulum-v1_lra1e-05_lrc0.005_g0.99_t0.02_h128_b256_n0.3_04182025_212106_.pth"
    # critic_path = "preTrained\ddpg_critic_Pendulum-v1_lra1e-05_lrc0.005_g0.99_t0.02_h128_b256_n0.3_04182025_212106_.pth"
