...
class SACAgent:
    ...
    def __init__(self, env: gym.Env, args=None):
        ...
        self.vf_optimizer = optim.Adam(self.vf.parameters(), lr=self.lr_v)
        self.qf_1_optimizer = optim.Adam(self.qf_1.parameters(), lr=self.lr_q)
        self.qf_2_optimizer = optim.Adam(self.qf_2.parameters(), lr=self.lr_q)
    ...
    def update_model(self) -> Tuple[torch.Tensor, ...]:
        ...
        with torch.no_grad():
            next_v_value = self.vf_target(next_state)
            q_target = reward + self.gamma * mask * next_v_value

        q1_value = self.qf_1(state, action)
        q2_value = self.qf_2(state, action)

        qf_1_loss = F.mse_loss(q1_value, q_target)
        qf_2_loss = F.mse_loss(q2_value, q_target)
        ...
        ########## Your Code (~5 lines)##########
        q1_new_value = self.qf_1(state, new_action)
        q2_new_value = self.qf_2(state, new_action)
        min_q = torch.min(q1_new_value, q2_new_value)

        vf_target = min_q - alpha * log_prob
        vf_loss = F.mse_loss(self.vf(state), vf_target.detach())
    ...
    def train(self):
        ...
        for self.total_step in tqdm(range(1, self.num_steps + 1)):
            ...
            if done:
                state, _ = self.env.reset(seed=self.seed)
                scores.append(score)
                ewma_score = 0.95 * ewma_score + 0.05 * score
                ewma_scores.append(ewma_score)
                ep += 1
                print(f"Episode {ep} (Total step = {self.total_step}): Total Reward = {score}, EWMA Reward = {ewma_score}")
                # W&B logging
                wandb.log({
                    "episode": ep,
                    "return": score,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lr_q", type=float, default=3e-3)
    ...
    # W&B init
    wandb.init(project="RL-HW3-SAC-HalfCheetah-v5", name=args.wandb_run_name, save_code=True)
